  ...
  local ref dirty mode repo_path
   ...
   if [[ "$(command git rev-parse --is-inside-work-tree 2>/dev/null)" = "true" ]]; then
    ...
    if [[ $AGNOSTER_GIT_BRANCH_STATUS == 'true' ]]; then
      local ahead behind upstream_counts
      upstream_counts=$(command git rev-list --left-right --count @{upstream}...HEAD 2>/dev/null)
...
agnoster_precmd() {
  print -P "%{%f%b%k%}%E$(build_prompt)%{%f%b%k%}%E"
}

precmd_functions=(${precmd_functions:#agnoster_precmd})
precmd_functions+=(agnoster_precmd)
PROMPT='%{%f%b%k%} '
