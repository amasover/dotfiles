...
Not introduced by this triage, but noticed: `main`'s `.gitconfig` has a work email (work domain — redacted; literal value in git history pending the BFG scrub) and production gitconfig includes; `.profile`/`.zshrc` reference `[work-ref]*` (work). These are company-internal details in a public repo — worth a pass through the pre-PR privacy workflow, tracked separately.
