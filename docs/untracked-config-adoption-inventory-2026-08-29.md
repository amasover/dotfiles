...
| `.local/bin/tools/hibernate` and `.config/systemd/user/locker.service` | 2 | i3 idle hibernation and logind lock integration | Adopted and made portable in Story 3.7 [#14](https://github.com/amasover/dotfiles/issues/14), PR [#221](https://github.com/amasover/dotfiles/pull/221): a tested timer launcher owns hibernation policy, and the tracked unit routes `lock.target` through the shared lock helper. |
