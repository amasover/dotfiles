...
Live audit: `.vimrc` is the only user-authored Vim config. vim-plug under
`~/.vim/autoload/` and clones under `~/.vim/plugged/` are generated by the
helper; `.viminfo` and `.vim/.netrwhist` are mutable history, not dotfiles.
