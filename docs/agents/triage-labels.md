...
Only `wontfix` exists on `amasover/dotfiles` today (a GitHub default). Create the other
four on first use:
...
`gh label create` fails if the label already exists; that failure is safe to ignore.
