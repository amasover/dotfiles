...
- Given a PR (and pushes to `main`), when CI runs, then the lint stage covers every tracked script: `shellcheck`, `shfmt -i 4 -bn -ci -d`, and Bash/sh syntax; `zsh -n`; `luac5.1 -p`; plus `actionlint` for workflow YAML and embedded shell
- Given the tracked suites — the shell regression suite (`clitest tests/*.clitest.txt`), the pytest suite (`pytest tests/`, the vm-harness display/leases/seed/trust/vmx units), the isolated Spacemacs config ERT suite (`emacs --batch -Q -l tests/spacemacs-config-test.el -f ert-run-tests-batch-and-exit`), and the older standalone pair (`.config/yay/hook-harness.lua`, `.config/dotfiles/tests/chaotic-quarantine-gate-test`) — when CI runs, then all execute and any case failure fails the check; suites stay host-independent (no libvirt, no network, no live-system state) per the runbook, and a suite that violates that gets fixed, not skipped
- Given the local pre-commit hook (4.4) can be absent (another machine, GitHub web edits), when CI runs, then betterleaks (built from the repo's own 2.46 vendored PKGBUILD, or its checksum-pinned release binary; gitleaks is an acceptable fallback) scans the pushed range as the second net
