...
- Given the tracked suites — the shell regression suite (`clitest tests/*.clitest.txt`), the pytest suite (`pytest tests/`, the vm-harness display/leases/seed/trust/vmx units), and the older standalone pair (`.config/yay/hook-harness.lua`, `.config/dotfiles/tests/chaotic-quarantine-gate-test`) — when CI runs, then all execute and any case failure fails the check; suites stay host-independent (no libvirt, no network, no live-system state) per the runbook, and a suite that violates that gets fixed, not skipped
