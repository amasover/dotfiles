...
- If a new sensitive file is needed, add the path pattern to `.config/yadm/encrypt` and regenerate the encrypted payload with YADM instead of committing plaintext. Adding the pattern is not enough on its own — until `yadm encrypt` runs, the path is declared but absent from the archive, and a fresh machine restores nothing. The `pre_push` guard (`.config/yadm/hooks/pre_push`, Story 4.9) blocks a `yadm push` that would ship that drift.
- Run a secret scan before staging commits and before pushing or opening a PR. The standard scanner is `betterleaks` (gitleaks' successor; reads the same configs — `gitleaks` remains the fallback on machines where betterleaks isn't installed yet); follow the [secret scan recipe](../knowledge/recipes/secret-scan.md), which also documents the manual fallback and false-positive handling. Keep scanner output redacted.
...
- Small docs-only bookkeeping changes may bypass a PR and go directly to `main`:
  ...
  generated or encrypted artifacts, and docs coupled to behavior changes.
...
- `main` is the trunk. Do not merge to `main` or push to a remote unless Aaron explicitly asks in the current task or the change fits the docs-only direct-main exception above.
...
- Run a secret scan (`betterleaks`, or the `gitleaks` fallback, per `knowledge/recipes/secret-scan.md`) before staging commits and before opening a PR. The pre-commit hook (`.githooks/pre-commit`) enforces the staged scan where enabled; the pre-PR working-tree scan stays manual.
- Always pair the scan with a manual privacy pass by eye (see Hard rule 2). The scanner finding nothing does not clear personal, employer, or machine-specific details — read the diff yourself.
